...
void pdcp::init(srsue::rlc_interface_pdcp *rlc_, srsue::lwipep_interface_pdcp* lwipep_, srsue::rrc_interface_pdcp *rrc_, srsue::gw_interface_pdcp *gw_, log *pdcp_log_, uint32_t lcid_, uint8_t direction_)
{
  rlc			= rlc_;
  lwipep		= lwipep_;
  rrc			= rrc_;
...
void pdcp::add_bearer(uint32_t lcid, srslte_pdcp_config_t cfg)
{
  ...
  if (!valid_lcid(lcid)) 
  {
    if (!pdcp_array.insert(pdcp_map_pair_t(lcid, new pdcp_entity())).second) 
	{
      pdcp_log->error("Error inserting PDCP entity in to array\n.");
      goto unlock_and_exit;
    }
    pdcp_array.at(lcid)->init(rlc, lwipep, rrc, gw, pdcp_log, lcid, cfg);
...
void pdcp::add_bearer_mrb(uint32_t lcid, srslte_pdcp_config_t cfg)
{
  pthread_rwlock_wrlock(&rwlock);
  if (!valid_mch_lcid(lcid)) {
    if (!pdcp_array_mrb.insert(pdcp_map_pair_t(lcid, new pdcp_entity())).second) {
      pdcp_log->error("Error inserting PDCP entity in to array\n.");
      goto unlock_and_exit;
    }
    pdcp_array_mrb.at(lcid)->init(rlc, lwipep, rrc, gw, pdcp_log, lcid, cfg);
    pdcp_log->info("Added bearer %s\n", rrc->get_rb_name(lcid).c_str());
  } else {
    pdcp_log->warning("Bearer %s already configured. Reconfiguration not supported\n", rrc->get_rb_name(lcid).c_str());
  }
...
void pdcp::change_lcid(uint32_t old_lcid, uint32_t new_lcid)
{
  ...
  if (valid_lcid(old_lcid) && !valid_lcid(new_lcid)) {
    ...
    if (!pdcp_array.insert(pdcp_map_pair_t(new_lcid, pdcp_entity)).second) {
