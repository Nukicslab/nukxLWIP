...
{
public:
	virtual void write_pdu(uint32_t lcid, srslte::byte_buffer_t *sdu, uint32_t net_if) = 0;						//gw <- lwip, receive from UE then send to EPC
...
{
public:
	virtual void write_pdu(uint32_t lcid, srslte::byte_buffer_t *pdu, uint32_t net_if) = 0;						//lwip <- pdcp, receive from UE then send to EPC
};
