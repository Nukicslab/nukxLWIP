...
//===================================================================================================//
	...
	void lwipep::write_pdu(uint16_t rnti, uint32_t lcid, srslte::byte_buffer_t* sdu, uint32_t net_if)
	{
		if (users.count(rnti)) 
		{	users[rnti].lwipep->write_pdu(lcid, sdu, net_if);	} 
		else 
		{	pool->deallocate(sdu);	}
	}
...
//===================================================================================================//
	...
	{
		for(std::map<uint32_t,user_interface>::iterator user = users.begin(); user != users.end(); user++)
			user->second.lwipep->set_lwipep_auto_control_ratio(dl_auto, ul_auto);
	}
