...
void pdcp::init(rlc_interface_pdcp* rlc_,  lwipep_interface_pdcp* lwipep_, rrc_interface_pdcp* rrc_, gtpu_interface_pdcp* gtpu_, srslte::log* pdcp_log_)
{
  rlc		= rlc_; 
  lwipep	= lwipep_;
  rrc		= rrc_; 
...
void pdcp::add_user(uint16_t rnti)
{
  ...
  if (users.count(rnti) == 0) {
    ...
    obj->init(&users[rnti].rlc_itf, &users[rnti].lwipep_itf, &users[rnti].rrc_itf, &users[rnti].gtpu_itf, log_h, RB_ID_SRB0, SECURITY_DIRECTION_DOWNLINK);
...
void pdcp::user_interface_lwipep::write_pdu(uint32_t lcid, srslte::byte_buffer_t *pdu, uint32_t net_if)
{
	lwipep->write_pdu(rnti, lcid, pdu, 1);
}
