...
#ifndef SRSENB_LWIPEP_XW_H
#define SRSENB_LWIPEP_XW_H
...
class lwipep_xw:	public thread,
					public srsenb::lwipep_xw_interface_lwipep
					//public srsenb::lwipep_xw_interface_lwipep_segw
{
public:
	lwipep_xw();
	void			init(lwipep_interface_lwipep_xw *lwipep_/*, lwipep_spgw_interface_xw *lwipep_spgw_,*/, srslte::log *lwipep_xw_log_);
	void			stop();
...
private:
	...
	int		socket_opt;
	struct	sockaddr_in lwipep_spgw_addr;		// lwipep_segw address
//	struct	timeval      metrics_time[3];
	...
	*/
	...
	bool	enable_lwipep_xw_rcv_pkt;
	bool	lwipep_xw_rcv_pkt_running;
	srslte::log					*lwipep_xw_log;
	srslte::byte_buffer_pool	*pool;
	static const int 	LWIPEP_XW_THREAD_PRIO = 7;
	lwipep_interface_lwipep_xw							*lwipep;
//	lwipep_segw_interface_lwipep_xw						*lwipep_segw;	
/*	
	class user_interface_lwipep : public srsenb::lwipep_interface_lwipep_xw
	{
	public:
		uint16_t rnti; 
		srsenb::lwipep_interface_lwipep_xw *lwipep; 
		void write_pdu(uint32_t lcid,  srslte::byte_buffer_t *sdu);
	};
	
	class user_interface_lwipep_spgw : public srsenb::lwipep_spgw_interface_lwipep_xw
	{
	public:
		uint16_t rnti; 
		srsenb::lwipep_spgw_interface_lwipep_xw *lwipep_spgw; 
		void write_sdu(uint32_t lcid,  srslte::byte_buffer_t *sdu);
	};
	...
	public: 
		...
		srsenb::lwipep_xw			*lwipep_xw; 
	};
	
	std::map<uint32_t,user_interface> users;
*/
//	void lwipep_xw_build_gtp_header(ip_header_t *ip_header, srslte::byte_buffer_t *sdu);
	
//	void lwipep_xw_build_ip_header(ip_header_t *ip_header, srslte::byte_buffer_t *sdu);
	
	void lwipep_xw_build_eth_header(ethhdr *eth_header);
	
	void lwipep_xw_write_eth_header(ethhdr *eth_header, srslte::byte_buffer_t *sdu);
	
	bool lwipep_xw_read_eth_header(srslte::byte_buffer_t *pdu, uint16_t *rnti, uint16_t *drbid);
...
/*
class lwipep_xw_interface_lwipep_segw: public thread
{
public:
	void			init();
	void			stop();
...
private:
	...
	std::map<uint32_t, xw_args_t>				users;
	srslte::byte_buffer_pool					*pool;
};
*/
}//srsenb
