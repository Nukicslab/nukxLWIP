...
//===================================================================================================//
	void stop();
	void reset_handle_report();
	void set_lwipep_auto_control_ratio();
	...
	void lost_pkt(control_radio_args *args, xw_rpt_pkt *rpt, uint32_t rpt_ptr, srslte::byte_buffer_t *pdu);
...
private:
	...
	int32_t					lwipep_config_time;
	bool					enable_config;
	bool					enable_interference_ratio;
	uint32_t				interference_ratio;
	uint32_t				last_bytes;

	control_radio_args		args;
	...
	bool		enable_auto_control_ratio;
...
//	const static float  rate_init_wlan	= 500000000; //500Mbps
	...
	uint32_t			over_current_max_sn_times;
	uint32_t            next_sn;
	uint32_t			delay_sn;
	...
	int32_t				lwipep_hdl_rpt_time;
	int32_t				lwipep_rcv_rpt_time;
	uint32_t			last_highest_xw_seq_num;	//24bits
	uint32_t			start_lte_tx_packet_count;
	uint32_t			start_wlan_tx_packet_count;
	...
	uint32_t            hdl_rpt_count;
	
	uint32_t half_lte_tx_ratio_count;
	uint32_t half_wlan_tx_ratio_count;
	//=================================================
	...
	srsue::lwipep_hdl_rpt_interface_lwipep	*lwipep_hdl_rpt;;		//xw or ipsec
...
//	void lwipep::write_pdu_aggregate(uint32_t lcid, srslte::byte_buffer_t* pdu);
	...
	route_t get_route(uint32_t bytes);
