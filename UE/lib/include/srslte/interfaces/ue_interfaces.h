...
//GW interface for LWIP
class gw_interface_lwipep
{
public:
	virtual void write_pdu(uint32_t lcid, srslte::byte_buffer_t *sdu) = 0;						//gw <- lwip, receive from UE then send to EPC
};

//RRC interface for LWIP
class rrc_interface_lwipep																		//add_tunnel
{
public:
};

//PDCP interface for LWIP
class pdcp_interface_lwipep
{
public:
	virtual void write_sdu(uint32_t lcid, srslte::byte_buffer_t *sdu, bool blocking) = 0;						//lwip -> pdcp, receive from EPC then send to UE
};

class rlc_interface_lwipep
{
public:
	virtual uint32_t get_buffer_state(uint32_t lcid) = 0;
};

//LWIP interface for GW
class lwipep_interface_gw
{
public:
	virtual void write_sdu(uint32_t lcid, srslte::byte_buffer_t *sdu, bool blocking) = 0;						//gtp -> lwip, receive from EPC then send to UE
};

//LWIP interface for LWIPEP_SeGW, LWIPEP_Xw, LWIPEP_IPsec_Tunnel
class lwipep_interface_lwipep_ipsec
{
public:
	virtual void write_pdu(uint32_t lcid, srslte::byte_buffer_t *sdu) = 0;						//gw <- lwip, receive from UE then send to EPC
	virtual void get_mac(uint8_t *DST_MAC0, uint8_t *DST_MAC1, uint8_t *DST_MAC2, uint8_t *DST_MAC3, uint8_t *DST_MAC4, uint8_t *DST_MAC5) = 0;
};

//LWIP interface for LWIPEP_SeGW, LWIPEP_Xw, LWIPEP_IPsec_Tunnel
class tunnel_interface_lwipep
{
public:
//	virtual void write_sdu(uint32_t lcid, srslte::byte_buffer_t *sdu) = 0;						//gtp -> lwip, receive from EPC then send to UE
	virtual void write_sdu(uint32_t lcid, srslte::byte_buffer_t *sdu) = 0;						//gtp -> lwip, receive from EPC then send to UE
	virtual void stop() = 0;
	virtual uint32_t get_buffer_state(uint32_t lcid) = 0;
	virtual void tunnel_speed_limit(uint32_t limit) = 0;
};

class lwipep_ipsec_interface_lwipep
{
public:
	virtual void write_sdu(uint32_t lcid, srslte::byte_buffer_t *sdu) = 0;						//gtp -> lwip, receive from EPC then send to UE
//	virtual void write_sdu(srslte::byte_buffer_t *sdu) = 0;						//gtp -> lwip, receive from EPC then send to UE
	virtual void stop() = 0;
	virtual void tunnel_speed_limit(uint32_t limit) = 0;
};

//LWIP interface for RRC
class lwipep_interface_rrc																		//add_tunnel
{
public:
...
//LWIP interface for PDCP
class lwipep_interface_pdcp
{
public:
	virtual void write_pdu(uint32_t lcid, srslte::byte_buffer_t *pdu) = 0;						//lwip <- pdcp, receive from UE then send to EPC
};

class lwipep_interface_phy
{
public:
	virtual void set_crnti(uint16_t rnti) = 0;						//lwip <- pdcp, receive from UE then send to EPC
};

class lwipep_hdl_rpt_interface_lwipep
{
public:
	virtual void handle_args(void *args) = 0;
	virtual void update_route(void *args) = 0;
	virtual void stop() = 0;						//lwip <- pdcp, receive from UE then send to EPC
};
//============================================================================================
