...
bool gtpu::init(std::string gtp_bind_addr_, 
				...
				srsenb::lwipep_interface_gtpu* lwipep_,
				srslte::log* gtpu_log_, bool enable_mbsfn)
{
  pdcp          = pdcp_;
  lwipep		= lwipep_;
  gtpu_log      = gtpu_log_;
  ...
  if(enable_mbsfn) {
    mchthread.init(m1u_multiaddr_, m1u_if_addr_, pdcp, lwipep, gtpu_log);
  }
  return true;
}
...
// gtpu_interface_pdcp -> gtpu_interface_lwipep
  ...
  while(run_enable) {
    ...
    switch(header.message_type) {
      ...
      case GTPU_MSG_DATA_PDU:
		...
		lwipep->write_sdu(rnti, lcid, pdu);
...
***************************************************************************/
bool gtpu::mch_thread::init(std::string m1u_multiaddr_, std::string m1u_if_addr_, pdcp_interface_gtpu *pdcp, lwipep_interface_gtpu* lwipep, srslte::log *gtpu_log)
{
  pool				= byte_buffer_pool::get_instance();
  this->pdcp		= pdcp;
  this->lwipep		= lwipep;
