...
void parse_args(all_args_t *args, int argc, char* argv[]) {
  ...
  if (vm.count("log.all_level")) {
	...
	if (!vm.count("log.lwipep_xw_level")) {
		args->log.lwipep_xw_level = args->log.all_level;
    }
  ...
  if (vm.count("log.all_hex_limit")) {
	...
	if (!vm.count("log.lwipep_xw_hex_limit")) {
		args->log.lwipep_xw_hex_limit = args->log.all_hex_limit;
    }
...
void *input_loop(void *m)
{
  ...
  while(running) {
	...
	else 
	{
	  ...
	  {	enb->set_lwipep_auto_control_ratio();	}
	  else if('w' == key || 'W' == key)
	  {
		  int dl, ul;
		  bool dl_wlan = false;
		  ...
		  cout << "enb->set_lwipep_wlan";
		  
		  enb->set_lwipep_wlan(dl_wlan, ul_wlan);
	  }
	  else if('s' == key || 'S' == key)
	  {
		  enb->lwipep_state();
	  ...
	  else if('d' == key || 'D' == key)
	  {
		  ...
		  enb->set_lwipep_tx_ratio(lte_tx_ratio, wlan_ratio);
	  }
	  /*
	  else if('u' == key || 'U' == key)
	  {
	  ...
	  */
	  else if('b' == key || 'B' == key)
	  {
		  ...
		  cin >> buffer_size;
		  enb->set_lwipep_reorder_buffer_size(buffer_size);
	  }
	  else if('r' == key || 'R' == key)
	  {
		  int report_period;
		  cout << "Enter reorder buffer size ratio:";
		  cin >> report_period;
		  enb->set_lwipep_report_period(report_period);
	  }
	  else if('t' == key || 'T' == key)
	  {
		  int reorder_timeout;
		  cout << "Enter reorder buffer size ratio:";
		  cin >> reorder_timeout;
		  enb->set_reorder_timeout(reorder_timeout);
	  }
	  else if('a' == key || 'A' == key)
	  {}
    }
