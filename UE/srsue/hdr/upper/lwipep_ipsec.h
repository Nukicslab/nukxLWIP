...
class lwipep_ipsec:	srslte::timer_callback,
					public thread,
					public lwipep_ipsec_interface_lwipep		//public srsue::lwipep_ipsec_interface_lwipep_segw
{
public:
	lwipep_ipsec();
	void			init(srsue::lwipep_interface_lwipep_ipsec *lwipep_, srslte::mac_interface_timers *mac_timers_, srslte::log *lwipep_ipsec_log_);
	void			stop();
	...
	void			tunnel_speed_limit(uint32_t limit);
	void			run_thread();
...
//	struct	timeval			metrics_time[3];
	...
	*/
	...
	srslte::timers::timer *lwipep_ipsec_limit_config;
	uint32_t lwipep_ipsec_limit_config_id;
	uint32_t lwipep_ipsec_limit_config_time;
	uint32_t next_sn;
	uint32_t buffer_sn;
	uint32_t interference_ratio;
	std::map<uint32_t, srslte::byte_buffer_t *> rx_window;
	std::map<uint32_t, srslte::byte_buffer_t *> tx_window;
	const static uint32_t BUFFER_SIZE = (1 << 8);
	bool enable_lwipep_ipsec_send_limit;
