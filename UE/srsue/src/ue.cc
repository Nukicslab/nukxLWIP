...
bool ue::init(all_args_t *args_) 
{
  ...
  lwipep_hdl_rpt_log.init("LWIPEP_HDL_RPT", logger);
  ...
  lwipep_hdl_rpt_log.set_level(level(args->log.lwipep_hdl_rpt_level));
  ...
  lwipep_hdl_rpt_log.set_hex_limit(args->log.lwipep_hdl_rpt_hex_limit);
  ...
  lwipep.init(&rlc, &pdcp, &lwipep_hdl_rpt, &lwipep_ipsec, &rrc, &mac, &gw, &lwipep_log, 3 /*RB_ID_SRB3*/);
  lwipep_ipsec.init(&lwipep,&mac, &lwipep_ipsec_log);
  lwipep_hdl_rpt.init(&mac, &lwipep_hdl_rpt_log);
...
void ue::set_lwipep_auto_control_ratio()
{
	lwipep.set_lwipep_auto_control_ratio();
	return;
}
