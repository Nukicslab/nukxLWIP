...
namespace srsue {
					  ...
					  srslte::log *lwipep_log_, uint32_t lcid_)
	{
		...
		tunnel->rlc = rlc;
		hdl_rpt = new lwipep_hdl_rpt_interface_lwipep;
		hdl_rpt->lwipep_hdl_rpt = lwipep_hdl_rpt;
		lwipep_entity->init(pdcp, hdl_rpt, tunnel, rrc, mac_timers, gw, lwipep_log, lcid, 0);
...
//		pthread_rwlock_init(&rwlock, NULL);
	...
	void lwipep::write_pdu(uint32_t lcid, srslte::byte_buffer_t* sdu)
	{	lwipep_entity->write_pdu(lcid, sdu);	}
...
//===================================================================================================//
	...
	{	lwipep_entity->set_lwipep_auto_control_ratio();	}
	...
	void lwipep::tunnel_interface_lwipep_ipsec::tunnel_speed_limit(uint32_t limit)
	{	lwipep_ipsec->tunnel_speed_limit(limit);	}
	...
	void lwipep::lwipep_hdl_rpt_interface_lwipep::handle_args(void *args)
	{
		lwipep_hdl_rpt->handle_args(args);
	}
	
	void lwipep::lwipep_hdl_rpt_interface_lwipep::update_route(void *args)
	{
		lwipep_hdl_rpt->update_route(args);
	}
	
	void lwipep::lwipep_hdl_rpt_interface_lwipep::stop()
	{
		lwipep_hdl_rpt->stop();
	}
